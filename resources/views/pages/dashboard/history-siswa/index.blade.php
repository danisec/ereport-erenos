    ...
    <section class="layout min-h-screen bg-hero py-6 font-rubik">
        ...
        <div class="relative overflow-x-auto rounded-2xl shadow-sm">
            <table class="w-full text-left">
                ...
                <div class="bg-white px-9 py-5">
                    <div class="flex h-14 flex-row items-center justify-between rounded-3xl bg-hero px-20">
                        <h4 class="text-2xl font-bold text-white">Data History Siswa</h4>
                        ...
                        <div class="flex flex-row gap-4">
                            ...
                            <a href="{{ route('historysiswa.create') }}">
                                <x-atoms.plus :alt="'tambah-historysiswa'" />
                            </a>
                ...
                @if ($historySiswa->count())
                    @foreach ($historySiswa as $index => $item)
                        <tbody>
                            <tr
                                class="{{ $index % 2 === 0 ? 'bg-white' : 'bg-blue-100' }} text-base font-medium leading-5">
                                <th class="px-9" scope="row">
                                    {{ ($historySiswa->currentPage() - 1) * $historySiswa->perPage() + $loop->iteration }}
                                </th>
                                <td class="pl-6">
                                ...
                                <td class="mr-3 flex flex-row items-center gap-5 2xl:m-0">
                                    <a href="{{ route('historysiswa.show', $item->idHistory) }}">
                                        <x-atoms.eye :alt="'detail-historysiswa'" />
                                    </a>

                                    <a href="{{ route('historysiswa.edit', $item->idHistory) }}">
                                        <x-atoms.pencil :alt="'edit-historysiswa'" />
                                    </a>

                                    <div x-data="{ show: false }">
                                        <button class="focus:outline-none" type="button" @click="show = !show">
                                            <x-atoms.trash :alt="'delete-historysiswa'" />
                                        </button>

                                        <x-molecules.modaldelete :title="'Apakah Anda akan menghapus history siswa : ' .
                                            $item->siswa->nmSiswa .
                                            ' ?'" :action="route('historysiswa.destroy', $item->idHistory)" />
                                    </div>
            ...
            <div class="bg-white p-6">
                {{ $historySiswa->links('vendor.pagination.tailwind') }}
            </div>
